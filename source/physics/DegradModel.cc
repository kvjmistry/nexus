...
void DegradModel::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep) {
    ...
    do {
        ...
        return_status = run_degrad(degrad_exec_str);
...
void DegradModel::GetElectronsFromDegrad(G4FastStep& fastStep, G4ThreeVector G4Pos, G4ThreeVector G4Dir, G4double G4Time, G4int trk_id)
{
    ...
    // To Do:
    // Get direction of particle generated by G4
    // get degrad direction, shoud be the same for all particles -- seems to be in some random but fixed direction. 
    // Apply coordinate transformation about detector center so that hits are in direction of G4 generated
    ...
    if (trj) {
        ...
        std::cout << "Total ionizations:" << GetTotIonizations(trk_id) << std::endl;
        G4double mean_ioni_E = GetAvgIoniEnergy(trk_id)*eV;
        trj->SetEnergyDeposit(mean_ioni_E * GetTotIonizations(trk_id)); // This overwrites the same track
...
// KM - This function allows us to run degrad multiple 
// times in the same function without it erroring out
// due to the multiple threads on the machine
// Code from a LLM, no idea how it works ;-)
G4int DegradModel::run_degrad(const char* cmd) {
    pid_t pid;
    char *argv[] = {
        (char*)"/bin/sh",
        (char*)"-c",
